...
export default function FeedbackPage(props: { params: Promise<{ token: string }> }) {
  ...
  // Resolve token → linkId + candidateName
  useEffect(() => {
      ...
      try {
        ...
        if (session?.candidate_id) {
          ...
          if (userRow?.full_name) {
            setCandidateName((userRow.full_name as string).split(" ")[0]);
          }
        }
  ...
  async function submit() {
    ...
    try {
      ...
      // Increment response count via RPC
  ...
  if (phase === "error") {
    return <CenteredCard>
      ...
      <p className="text-center" style={{ color: "var(--app-secondary)" }}>{errorMsg}</p>
    </CenteredCard>;
  }
  ...
  if (phase === "done") {
    return <CenteredCard>
      ...
      <h2 className="text-xl font-bold text-center" style={{ color: "var(--app-primary)" }}>
        {t("Vielen Dank!","Thank you!",isGerman)}
      </h2>
      <p className="text-sm text-center" style={{ color: "var(--app-secondary)" }}>
        {t(`Dein Feedback für ${candidateName || "den Kandidaten"} wurde erfolgreich übermittelt.`,
           `Your feedback for ${candidateName || "the candidate"} has been successfully submitted.`,isGerman)}
      </p>
    </CenteredCard>;
  }
        ...
        <div className="max-w-sm w-full space-y-6 fade-in">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm" style={{ color: "var(--app-secondary)" }}>
              {candidateName
                ? t(`${candidateName} bereitet sich auf ein Pilotenauswahlverfahren vor und bittet dich um eine ehrliche, anonyme Einschätzung.`,
                     `${candidateName} is preparing for a pilot assessment and asks for your honest, anonymous feedback.`,isGerman)
            ...
            <p className="text-xs" style={{ color: "var(--app-tertiary)" }}>
              {t("Ca. 3–5 Minuten • Anonym • Keine Registrierung nötig",
                 "~3–5 minutes • Anonymous • No registration needed",isGerman)}
            </p>
          </div>
          ...
          <div className="card p-4 space-y-4">
            <div>
              <label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--app-secondary)" }}>
                {t("Dein Vorname (optional)","Your first name (optional)",isGerman)}
              </label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                placeholder={t("z.B. Maria","e.g. John",isGerman)}
                className="input-field" />
            </div>
            <div>
              <label className="text-xs font-semibold mb-2 block" style={{ color: "var(--app-secondary)" }}>
                {t("Wie kennst du diese Person?","How do you know this person?",isGerman)}
              </label>
              <div className="grid grid-cols-2 gap-2">
                {(Object.entries(RELATIONSHIP_LABELS) as [RelationshipType, { de: string; en: string }][]).map(([rel, labels]) => (
                  <button key={rel} onClick={() => setRelationship(rel)}
                    ...
                    style={{
                      background: relationship === rel ? "rgba(74,158,248,0.15)" : "var(--app-input)",
                      border: `1px solid ${relationship === rel ? "#4A9EF8" : "transparent"}`,
                      color: relationship === rel ? "#4A9EF8" : "var(--app-secondary)",
                    }}>
          ...
          <button className="btn-primary" onClick={() => setPhase("survey")}>
            {t("Fragebogen starten","Start questionnaire",isGerman)}
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-svh flex items-center justify-center px-5" style={{ background: "var(--app-bg)" }}>
        <div className="max-w-sm w-full space-y-5 fade-in">
          <h2 className="text-xl font-bold text-center" style={{ color: "var(--app-primary)" }}>
            {t("Fast fertig!","Almost done!",isGerman)}
          </h2>
          <div className="card p-5 space-y-5">
            ...
            <div>
              <p className="text-sm font-semibold mb-3" style={{ color: "var(--app-primary)" }}>
                {candidateName
                  ? t(`Wie sehr glaubst du, dass ${candidateName} das Assessment bestehen wird?`,
                       `How confident are you that ${candidateName} will pass the assessment?`,isGerman)
                  : t("Wie sehr glaubst du, dass die Person das Assessment bestehen wird?",
                       "How confident are you that this person will pass the assessment?",isGerman)}
              </p>
              <div className="flex justify-between">
                {[1,2,3,4,5].map((i) => (
                  <button key={i} onClick={() => setConfidenceRating(i)}
                    className={`rating-circle ${i <= confidenceRating ? "active" : ""}`}>
                    {i}
                  </button>
                ))}
              </div>
              <div className="flex justify-between text-xs mt-2" style={{ color: "var(--app-tertiary)" }}>
                <span>{t("Unwahrscheinlich","Unlikely",isGerman)}</span>
                <span>{t("Sehr wahrscheinlich","Very likely",isGerman)}</span>
              </div>
            </div>
            ...
            <div>
              <p className="text-sm font-semibold mb-2" style={{ color: "var(--app-primary)" }}>
                {candidateName
                  ? t(`Hast du eine persönliche Botschaft oder Wunsch für ${candidateName}?`,
                       `Do you have a personal message or wish for ${candidateName}?`,isGerman)
              ...
              <textarea value={wishText} onChange={(e) => setWishText(e.target.value)}
                placeholder={t("z.B. Ich wünsche dir viel Erfolg!","e.g. I wish you all the best!",isGerman)}
                rows={3} className="w-full resize-none rounded-xl p-3.5 text-sm outline-none"
          ...
          <button className="btn-primary" onClick={submit} disabled={submitting}>
            {submitting ? <div className="spinner" /> : t("Feedback absenden","Submit feedback",isGerman)}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-svh flex flex-col" style={{ background: "var(--app-bg)" }}>
      ...
      <div className="sticky top-0 z-10 px-4 pb-3 space-y-2"
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <div className="text-xs font-semibold" style={{ color: "#4A9EF8" }}>
              {isGerman ? q.sectionTitle : q.sectionTitleEN}
            </div>
      ...
      <div className="px-4" style={{ paddingBottom: "max(env(safe-area-inset-bottom, 16px), 16px)" }}>
        <button
          onClick={next}
          disabled={q.type !== "openText" && !answers[q.id]}
...
function CenteredCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-center min-h-svh px-5" style={{ background: "var(--app-bg)" }}>
      <div className="card max-w-sm w-full p-8 space-y-4 fade-in">
        {children}
      </div>
    </div>
  );
